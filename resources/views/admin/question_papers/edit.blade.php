        ...
        <div class="card">
            <div class="card-header d-flex justify-content-between">
                <div class="header-title">
                    <h4 class="card-title">Edit Question Paper: {{ $questionPaper->name }}</h4>
                </div>
            </div>
            <div class="card-body">
                @if($errors->any())
                ...
                @endif
                <form action="{{ route('admin.question_papers.update', $questionPaper->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="form-group">
                        <label for="name">Question Paper Name</label>
                        <input type="text" class="form-control" id="name" name="name" value="{{ old('name', $questionPaper->name) }}" required>
                    </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Pre-populate selected questions for edit mode
    const initialSelectedQuestions = @json($selectedQuestions);
    for (const qId in initialSelectedQuestions) {
        const question = initialSelectedQuestions[qId];
        renderSelectedQuestion(question.id, question.question_text, question.pivot.marks);
        // Hide from available list initially
        const originalQuestion = availableQuestionsList.querySelector(`.draggable-question[data-id="${question.id}"]`);
        if (originalQuestion) {
            originalQuestion.style.display = 'none';
        }
    }
    ...
    function renderSelectedQuestion(questionId, questionText, initialMarks = 1) {
        ...
        li.querySelector('.remove-selected-question').addEventListener('click', function() {
            ...
            applyFilterAndHideSelected(); // Re-apply filter after removal
        });
        ...
        // Hide the question from the available list immediately after rendering it as selected
        const originalQuestion = availableQuestionsList.querySelector(`.draggable-question[data-id="${questionId}"]`);
        if (originalQuestion) {
            originalQuestion.style.display = 'none';
        }
    ...
    function applyFilterAndHideSelected() {
        ...
        document.querySelectorAll('.draggable-question').forEach(question => {
            ...
            const isSelected = selectedQuestionsData[question.dataset.id];

            if (isSelected) {
                question.style.display = 'none';
            } else if (selectedSubjectId === '' || questionSubjectId === selectedSubjectId) {
    ...
    subjectFilter.addEventListener('change', applyFilterAndHideSelected);

    // Initial filter application and hide already selected questions
    applyFilterAndHideSelected();
});
</script>
@endpush
